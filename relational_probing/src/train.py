...
# ── Helpers ───────────────────────────────────────────────────────────────────
def embed_strings(encoder: ModernBERTEmbeddings, texts: list[str], batch_size: int = 64) -> torch.Tensor:
...
def load_or_compute(encoder: ModernBERTEmbeddings, texts: list[str], cache_path: str) -> torch.Tensor:
...
def main() -> None:
    ...
    # --- Encoder ---
    encoder = ModernBERTEmbeddings(config['model_name'])
    encoder.model.eval()
