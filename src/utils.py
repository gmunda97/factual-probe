...
class UtilityFunctions:
    ...
    @staticmethod
    def plot_losses_relational_probing(train_losses: list, val_losses: list, output_file: str) -> None:
        plt.figure(figsize=(5, 4))
        plt.plot(train_losses, label='Training Loss')
        plt.plot(val_losses, label='Validation Loss')
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Cosine Distance Loss', fontsize=12)
        plt.title("Training loss: $1 - cos(W\\hat{o}, h_o)$")
        plt.xticks(fontsize=12)
        plt.yticks(fontsize=12)
        plt.legend(fontsize=12)
        plt.tight_layout()
    ...
    def plot_all_losses(all_train_losses: dict, all_val_losses: dict) -> None:
        ...
        plt.legend()
        plt.savefig('./../resources/plots/training/wiki_desc/reduced_dim/all_losses.png')
